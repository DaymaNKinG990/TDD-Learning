...
### 🎮 Практическое упражнение: RED фаза

Попробуйте сами! Напишите тест для функции `add`, которая должна складывать два числа:

{{ code_input_form(
    exercise_id="red_phase_add",
    initial_code="""# Шаг 1: Напишите тест (RED фаза)
# Тест должен проверить, что функция add(2, 3) возвращает 5

def test_add():
    # Ваш тест здесь
    pass
""",
    placeholder="Напишите тест для функции add",
    use_pyodide=True,
    test_cases=[
        {
            "code": "test_add()",
            "description": "Тест должен вызывать функцию add(2, 3) и проверять результат"
        }
    ]
) }}

!!! tip "Подсказка"
    В RED фазе мы пишем тест **до** реализации функции. Тест должен падать, потому что функции `add` еще не существует!
...
### 🎮 Практическое упражнение: GREEN фаза

Реализуйте функцию `add`, чтобы тест прошел. Помните: в GREEN фазе можно использовать хардкод!

{{ code_input_form(
    exercise_id="green_phase_add",
    initial_code="""# Шаг 2: Реализуйте функцию add (GREEN фаза)
# Цель: сделать тест зеленым любой ценой!

def add(a, b):
    # Ваша реализация здесь
    # Подсказка: можно использовать хардкод для первого теста!
    pass
...
""",
    ...
    test_cases=[
        {
            "code": "test_add()",
            "description": "Тест должен пройти - функция add(2, 3) возвращает 5"
        }
    ]
) }}

!!! tip "Подсказка"
    В GREEN фазе можно написать `return 5` - это нормально! Главное, чтобы тест прошел.
